...
def get_bpx_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["bpx", "version"], capture_output=True)
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/bpx-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/bpx-gui/packages/gui/package.json")
